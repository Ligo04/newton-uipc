...
class TestStablePDActuatorUsdParsing(unittest.TestCase):
    ...
    :class:`DriveStablePD`, including the StablePD-specific ``numWorlds``
    ...
    def test_parse_stable_pd_from_usd(self):
        """NewtonStablePDControlAPI resolves to DriveStablePD with its params."""
        ...
        self.assertIsNotNone(parsed)
        self.assertIsInstance(parsed, ActuatorParsed)
        self.assertEqual(parsed.drive_class, DriveStablePD)
        self.assertAlmostEqual(parsed.drive_kwargs["kp"], 300.0)
        self.assertAlmostEqual(parsed.drive_kwargs["kd"], 30.0)
        self.assertEqual(parsed.target_path, "/World/Robot/Joint1")
    ...
    def test_parse_stable_pd_num_worlds(self):
        ...
        parsed = parse_actuator_prim(prim)
        self.assertEqual(parsed.drive_class, DriveStablePD)
        self.assertEqual(parsed.drive_kwargs["num_worlds"], 4)

    def test_finalize_stable_pd_from_usd(self):
        """Full parse_usd path builds a DriveStablePD actuator after finalize."""
        stage = self._build_stable_pd_stage()
        builder = newton.ModelBuilder()
        ...
        self.assertEqual(len(model.actuators), 1)
        act = model.actuators[0]
        self.assertIsInstance(act.drive, DriveStablePD)
        self.assertAlmostEqual(act.drive.kp.numpy()[0], 300.0, places=3)
        self.assertAlmostEqual(act.drive.kd.numpy()[0], 30.0, places=3)
...
class TestDriveStablePD(unittest.TestCase):
    ...
    def _make(self, device, num_worlds=1):
        ...
        ctrl = DriveStablePD(kp=kp, kd=kd, num_worlds=num_worlds)
        ctrl.finalize(device, n)
        return ctrl, n
